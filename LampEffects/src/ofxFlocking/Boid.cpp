...
void Boid::update(vector<Boid> &boids) {
    ...
    float force = 0.5;
    if(loc.x < border){
        acc += ofVec2f((border-loc.x)/border,0)*force;
    }
    if(loc.y < border){
        acc += ofVec2f(0,(border-loc.y)/border)*force;
    }
    if(loc.x > ofGetWidth() - border){
        acc += ofVec2f(-(border-(ofGetWidth()-loc.x))/border,0)*force;
    }
    if(loc.y > ofGetHeight() - border){
        acc += ofVec2f(0,-(border-(ofGetHeight()-loc.y))/border)*force;
    }
    ...
    loc += vel;
    acc = ofVec2f(0);  // Reset accelertion to 0 each cycle
